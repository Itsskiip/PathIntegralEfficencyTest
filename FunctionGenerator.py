...
from sympy import sympify, symbols, re, simplify, Number, Expr, nan
import numpy as np
import json
...
def construct_random_scalar_functions_branching(params: int | list, weighted=True, max_depth=10,
    ...
    branches = set()
    func = random_function(weighted)
    for _ in range(max(1, round(branches_per_layer(max_depth)))):
        inputs = []
        m = 1
...
def prune_function(function: Expr, prune_constants=True, complex_functions=False, round_n=2) -> Expr | None:
    try:
        ...
        if func_new == nan or (
                prune_constants and not func_new.free_symbols):
            return None
...
def prune_function_list(function_list: list | set, prune_constants=True, complex_functions=False,
                        round_n=2, max_wait=0.5, num_processes: int | None = None) -> list | set:
    prune_fn_partial = functools.partial(prune_function, prune_constants=prune_constants,
    ...
    new_function_list = {r for r in results if r}
    if type(function_list) is list:
        return list(new_function_list)
...
def generate_dataset(num_functions: int, min_num_trees: int, directory_path: str,
                     ...
                     constant_chance=lambda d: (10-d)/20, constant_range=(-10,10),
                     single_variable_chance=lambda d: (10-d)/20, complex_functions=False,
                     prune_constants=True, round_n: int = 3, max_wait: float = 0.5, num_processes: None | int = 4):

    dataset = set()
    # for _ in tqdm(range(min_num_trees), desc="Generating"):
    print(f"Generating {min_num_trees} function-trees")
    trees = 0
    while trees < min_num_trees or len(dataset) < num_functions:
        print(f"Building tree {trees+1}/{min_num_trees}")
        unpruned = construct_random_scalar_functions_branching(params, weighted, max_depth, branches_per_layer=branches_per_layer,
        ...
        print(f"\rPruning {len(unpruned)} functions")
        dataset |= prune_function_list(unpruned, prune_constants=prune_constants, complex_functions=complex_functions, round_n=round_n, max_wait=max_wait, num_processes=num_processes)
    ...
    print('\n'.join(str(expr) for expr in dataset))
